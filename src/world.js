...
function _buildLabRoom(scene) {
  ...
  const wallMat  = new THREE.MeshLambertMaterial({ color: 0xdde3ec });
  const ceilMat  = new THREE.MeshLambertMaterial({ color: 0xf5f5f8 });
  const darkMat  = new THREE.MeshLambertMaterial({ color: 0x2a2f3d });
  ...
  for (const [lx, , lz] of [
    ...
    scene.add(housing);
    // Glowing tube
    const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 5.6, 8), tubeMat);
...
function _buildLabFurniture(scene) {
  const benchTopMat  = new THREE.MeshLambertMaterial({ color: 0xf0f4e8 }); // light worktop
  const benchBodyMat = new THREE.MeshLambertMaterial({ color: 0x4a5568 }); // dark cabinet body
  const shelfMat     = new THREE.MeshLambertMaterial({ color: 0xd0d8e8 });
  ...
  for (const { x, z } of benchData) {
    ...
    if (x === 0) {
      const sinkMat  = new THREE.MeshLambertMaterial({ color: 0xccddee });
      const sink = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.2, 0.9), sinkMat);
      sink.position.set(2.5, 3.15, z);
      scene.add(sink);
      const tap = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.6, 8), metalMat);
      tap.position.set(2.5, 3.55, z - 0.3);
      scene.add(tap);
      const tapHead = new THREE.Mesh(new THREE.SphereGeometry(0.09, 8, 8), metalMat);
  ...
  const hoodBodyMat = new THREE.MeshLambertMaterial({ color: 0x6a7a8a });
  const hoodGlassMat = new THREE.MeshLambertMaterial({ color: 0x88ccdd, transparent: true, opacity: 0.4 });
...
function _buildLabDetails(scene) {
  ...
  const liquidMat   = new THREE.MeshLambertMaterial({ color: 0x3d8b40, transparent: true, opacity: 0.85 });
  const brownLiqMat = new THREE.MeshLambertMaterial({ color: 0x7a3c10, transparent: true, opacity: 0.85 });
  const redLiqMat   = new THREE.MeshLambertMaterial({ color: 0xc0392b, transparent: true, opacity: 0.85 });
  const whiteMat    = new THREE.MeshLambertMaterial({ color: 0xffffff });
  const metalMat    = new THREE.MeshLambertMaterial({ color: 0x8899aa });
  const labelMat    = new THREE.MeshLambertMaterial({ color: 0xfff5cc });
  ...
  for (const b of bottleData) {
    const bottle = new THREE.Mesh(new THREE.CylinderGeometry(b.r, b.r * 0.9, b.h, 10), b.mat);
    bottle.position.set(b.x, b.y + b.h / 2, b.z);
    scene.add(bottle);
    // Cap
    const cap = new THREE.Mesh(new THREE.CylinderGeometry(b.r * 0.7, b.r * 0.7, 0.12, 10), whiteMat);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const bk = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.2, 0.5, 10), bkMat);
  ...
  const hangerPost = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 1.5, 8), metalMat);
  hangerPost.position.set(-LAB_W/2 + 0.2, 9, 14);
  hangerPost.rotation.z = Math.PI / 2;
  scene.add(hangerPost);
  for (const hx of [-0.5, 0, 0.5]) {
    const hook = new THREE.Mesh(new THREE.TorusGeometry(0.14, 0.03, 8, 12, Math.PI), metalMat);
  ...
  for (const [bx, bz, bm] of [[-8, 14, binMat], [8, 14, bin2Mat]]) {
    const bin = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.38, 1.0, 10), bm);
  ...
  const ext = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.22, 1.1, 10), extMat);
  ext.position.set(LAB_W/2 - 0.8, 1.8, -14);
  scene.add(ext);
  const extTop = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.2, 0.3, 10), metalMat);
  ...
  scene.add(clock);
  const clockRim = new THREE.Mesh(new THREE.TorusGeometry(0.7, 0.06, 8, 24), metalMat);
...
function _addCentrifuge(scene, x, y, z, metalMat, whiteMat) {
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.65, 0.7, 0.7, 14), whiteMat);
  body.position.set(x, y + 0.35, z);
  scene.add(body);
  const lid = new THREE.Mesh(new THREE.CylinderGeometry(0.66, 0.66, 0.12, 14), metalMat);
  lid.position.set(x, y + 0.76, z);
  scene.add(lid);
  const knob = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), metalMat);
...
function _addMicroscope(scene, x, y, z, metalMat, whiteMat) {
  ...
  scene.add(arm);
  const eyepiece = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.12, 0.5, 8), metalMat);
  eyepiece.position.set(x, y + 1.7, z - 0.15);
  eyepiece.rotation.z = 0.4;
  scene.add(eyepiece);
  const objective = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.08, 0.35, 8), whiteMat);
  objective.position.set(x + 0.05, y + 0.75, z + 0.05);
  scene.add(objective);
}

function _addHotPlate(scene, x, y, z, metalMat) {
  const plateMat = new THREE.MeshLambertMaterial({ color: 0x2c3e50 });
  ...
  const flask = new THREE.Mesh(new THREE.SphereGeometry(0.3, 12, 12), flaskMat);
  flask.position.set(x, y + 0.55, z);
  scene.add(flask);
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.12, 0.35, 10), flaskMat);
...
export function createQuestionObjects(scene) {
  ...
  // ── Fenomena 1 – Water Quality Analysis Bench ──────────
  {
    ...
    group.add(rack);
    for (let i = 0; i < 5; i++) {
      const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.07, 0.55, 8), brownLiq);
      tube.position.set(-2.2 + i * 0.42, 3.62, 0.2);
      group.add(tube);
    }

    // Erlenmeyer flask with brown liquid (vinasse water sample)
    const flaskBody = new THREE.Mesh(new THREE.SphereGeometry(0.38, 12, 12), glassMat);
    flaskBody.position.set(0.8, 3.62, -0.3);
    group.add(flaskBody);
    const flaskNeck = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.15, 0.45, 10), glassMat);
    flaskNeck.position.set(0.8, 4.05, -0.3);
    group.add(flaskNeck);
    const liquidFill = new THREE.Mesh(new THREE.SphereGeometry(0.32, 10, 10), brownLiq);
    liquidFill.position.set(0.8, 3.58, -0.3);
    group.add(liquidFill);

    // DO meter probe (vertical rod going into beaker)
    const probe = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 1.0, 8), metalMat);
  ...
  // ── Fenomena 2 – TDS / Ion Analysis Bench ─────────────
  {
    ...
    group.add(tdsScreen);
    const tdsProbe = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.5, 8), metalMat);
    tdsProbe.position.set(-2.0, 3.05, 0);
    group.add(tdsProbe);

    // Large beaker with brown vinasse
    const beaker = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.38, 0.8, 12), glassMat);
    beaker.position.set(-0.5, 3.7, 0.2);
    group.add(beaker);
    const liq = new THREE.Mesh(new THREE.CylinderGeometry(0.38, 0.34, 0.6, 12), brownLiq);
    ...
    const stand = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.4, 6), metalMat);
    ...
    for (const [bx, bm] of [[2.8, brownLiq], [3.3, glassMat]]) {
      const bottle = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.16, 0.7, 10), bm);
  ...
  // ── Fenomena 3 – Rice / Soil Analysis Bench ───────────
  {
    ...
    for (const col of trayData) {
      ...
      for (let s = -1; s <= 1; s += 2) {
        ...
        const panicle = new THREE.Mesh(
          new THREE.SphereGeometry(col.height > 0.8 ? 0.14 : 0.06, 6, 5), col.mat);
    ...
    group.add(phBody);
    const phProbe = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 0.55, 8), metalMat);
    ...
    const soilBeaker = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.25, 0.6, 12), glassMat);
    soilBeaker.position.set(2.8, 3.55, -0.4);
    group.add(soilBeaker);
    const soilLiq = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.22, 0.38, 12), brownLiq);
