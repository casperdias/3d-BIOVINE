// ─────────────────────────────────────────────────────────────────────────────
// experimentUI.js – Optional form to upload real-life experiment results
// Submits to Google Sheets "Kelompok" sheet (fire-and-forget via no-cors)
// ─────────────────────────────────────────────────────────────────────────────

import { state }                  from './state.js';
import { uploadExperimentResult } from './sheets.js';

const $ = id => document.getElementById(id);

const OV_ID  = 'experiment-form-overlay';
const CSS_ID = 'experiment-form-css';
...
function obsRow(prefix) {
  const cols = ['warna','bau','keruh','ph','sal','tds','cod','bod','do'];
  return cols.map(c => `<td><input type="text" class="exp-cell" id="${prefix}-${c}" /></td>`).join('');
}

// ── Main export ───────────────────────────────────────────────────────────────
export function showExperimentForm() {
  injectCSS();
  removeOverlay();
  ...
  // ── Wire up close ────────────────────────────────────────────────────────
  $('exp-close').onclick = removeOverlay;
  ov.addEventListener('click', e => { if (e.target === ov) removeOverlay(); });

  // ── Wire up submit ───────────────────────────────────────────────────────
  $('exp-submit').onclick = () => {
    const nomorKelompok = $('exp-kelompok').value.trim();
    const kelas         = $('exp-kelas').value.trim();

    if (!nomorKelompok || !kelas) {
      showStatus('⚠️ Nomor Kelompok dan Kelas wajib diisi!', false);
      return;
    }
    ...
    function readRow(prefix) {
      return Object.fromEntries(
        cols.map(c => [c, $(`${prefix}-${c}`)?.value.trim() || '–'])
      );
    }
    ...
    const payload = {
      nomorKelompok,
      kelas,
      namaAnggota:      $('exp-nama').value.trim() || '–',
      playerName:       state.playerName || '–',
      timestamp:        new Date().toISOString(),
      // Table 1
      mikroorganisme:   $('t1-mikro').value.trim()  || '–',
      intensitasCahaya: $('t1-cahaya').value.trim() || '–',
      suhuRuang:        $('t1-suhu').value.trim()   || '–',
      // Table 2 – awal
      awalDengan: readRow('awal-dg'),
      awalTanpa:  readRow('awal-tp'),
      // Table 3 – akhir
      akhirDengan: readRow('akhir-dg'),
      akhirTanpa:  readRow('akhir-tp'),
    };
    ...
    uploadExperimentResult(payload)
      .then(() => {
        showStatus('✅ Berhasil! Data percobaan kelompokmu telah dikirimkan.', true);
        btn.textContent = '✅ Terkirim';
      })
      .catch(() => {
        showStatus('❌ Gagal mengirim. Periksa koneksi internet dan coba lagi.', false);
        btn.disabled    = false;
  ...
  function showStatus(msg, ok) {
    const el = $('exp-status');
    el.className = 'exp-status ' + (ok ? 'ok' : 'err');
    el.innerHTML  = msg;
