...
function _buildFactoryRoom(scene) {
  const concreteMat = new THREE.MeshLambertMaterial({ color: 0x5a5243 });
  const wallMat     = new THREE.MeshLambertMaterial({ color: 0x4a4030 });
  const roofMat     = new THREE.MeshLambertMaterial({ color: 0x2e261a });
...
function _buildFermentor(scene, x, y, z) {
  ...
  const drum = new THREE.Mesh(new THREE.CylinderGeometry(2, 2, 5.5, 16), drumMat);
  drum.position.y = 3.15;
  grp.add(drum);

  // Drum top dome
  const dome = new THREE.Mesh(new THREE.SphereGeometry(2, 16, 8, 0, Math.PI * 2, 0, Math.PI / 2), drumMat);
  dome.position.y = 5.9;
  grp.add(dome);

  // Drum bottom dome
  const domeBot = new THREE.Mesh(new THREE.SphereGeometry(2, 16, 8, 0, Math.PI * 2, Math.PI / 2, Math.PI / 2), drumMat);
  ...
  for (let ry = 1.5; ry <= 5.5; ry += 1.5) {
    const rib = new THREE.Mesh(new THREE.TorusGeometry(2.06, 0.09, 6, 24), ribMat);
  ...
  // Inlet pipe (top)
  const pipeIn = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 2, 8), pipeMat);
  pipeIn.position.set(0.8, 7.2, 0);
  grp.add(pipeIn);
  // Outlet pipe (bottom side)
  const pipeOut = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.15, 1.8, 8), pipeMat);
  ...
  grp.add(elbow);
  // Pipe going down
  const pipeDn = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.15, 1.2, 8), pipeMat);
  pipeDn.position.set(-3.7, 0.6, 0);
  grp.add(pipeDn);

  // Valve
  const valveMat = new THREE.MeshStandardMaterial({ color: 0xcc2222, metalness: 0.5, roughness: 0.5 });
  const valve = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.5, 8), valveMat);
  valve.rotation.z = Math.PI / 2;
  valve.position.set(-2.85, 2.5, 0);
  grp.add(valve);

  // CO2 vent pipe (top side)
  const ventPipe = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 2.5, 8), pipeMat);
  ...
  for (let i = 0; i < 5; i++) {
    const b = new THREE.Mesh(new THREE.SphereGeometry(0.12 + i * 0.04, 6, 6), bubbleMat);
...
function _buildDestilator(scene, x, y, z) {
  ...
  // Main column
  const column = new THREE.Mesh(new THREE.CylinderGeometry(0.8, 1.0, 8, 16), metalMat);
  column.position.y = 4.4;
  grp.add(column);

  // Column sections (bubble caps / trays)
  for (let cy = 1.5; cy <= 7.5; cy += 1.2) {
    const tray = new THREE.Mesh(new THREE.CylinderGeometry(0.95, 0.95, 0.12, 16), darkMetal);
    tray.position.y = cy;
    grp.add(tray);
  }

  // Condenser coil at top
  const condBase = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 0.7, 1.5, 12), darkMetal);
  ...
  for (let ci = 0; ci < 4; ci++) {
    const coil = new THREE.Mesh(new THREE.TorusGeometry(0.5, 0.06, 8, 20), coilMat);
  ...
  // Condenser top cap
  const cap = new THREE.Mesh(new THREE.SphereGeometry(0.7, 10, 6, 0, Math.PI * 2, 0, Math.PI / 2), darkMetal);
  cap.position.y = 10.2;
  grp.add(cap);

  // Feed pipe (vinasse in)
  const feedPipe = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 2.5, 8), pipeMat);
  feedPipe.rotation.z = Math.PI / 2;
  feedPipe.position.set(-1.85, 6.5, 0);
  grp.add(feedPipe);

  // Steam riser pipe at top
  const steamPipe = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 1.5, 8), pipeMat);
  steamPipe.position.set(0.5, 11, 0);
  grp.add(steamPipe);

  // Ethanol output pipe (side)
  const ethPipe = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 2, 8), pipeMat);
  ethPipe.rotation.z = Math.PI / 2;
  ethPipe.position.set(1.8, 9.6, 0);
  grp.add(ethPipe);

  // Vinasse waste pipe (bottom)
  const wastePipe = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 1.4, 8), pipeMat);
  ...
  const barrel    = new THREE.Mesh(new THREE.CylinderGeometry(0.65, 0.6, 1.2, 12), barrelMat);
  barrel.position.set(0.8, -0.2, 0);
  grp.add(barrel);
  const barrelLid = new THREE.Mesh(new THREE.CylinderGeometry(0.66, 0.66, 0.08, 12), darkMetal);
  ...
  const indicator = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.07, 4, 8), glassMat);
  indicator.position.set(-0.92, 3, 0.7);
  grp.add(indicator);
  const liquidLevel = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 2.5, 8),
...
function _buildKomporPemanas(scene, x, y, z) {
  ...
  // Burner fire glow
  const fire1 = new THREE.Mesh(new THREE.ConeGeometry(0.5, 1.2, 8), hotMat);
  fire1.position.y = 2.9;
  grp.add(fire1);
  const fire2 = new THREE.Mesh(new THREE.ConeGeometry(0.3, 0.9, 8), hotMat.clone());
  fire2.position.set(0.6, 2.85, 0.4);
  grp.add(fire2);

  // Boiling pot on top
  const pot = new THREE.Mesh(new THREE.CylinderGeometry(1.2, 1.0, 1.4, 16), panMat);
  pot.position.y = 3.75;
  grp.add(pot);

  // Pot contents (liquid)
  const liquidMat = new THREE.MeshStandardMaterial({ color: 0x7c4a10, transparent: true, opacity: 0.85 });
  const liquid    = new THREE.Mesh(new THREE.CylinderGeometry(1.18, 1.18, 0.8, 16), liquidMat);
  ...
  for (let s = 0; s < 3; s++) {
    const steam = new THREE.Mesh(new THREE.SphereGeometry(0.25 + s * 0.12, 6, 6), steamMat);
  ...
  const gauge    = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.08, 12), gaugeMat);
  ...
  const chimney    = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.3, 3, 10), chimneyMat);
  ...
  for (let sm = 0; sm < 4; sm++) {
    const smoke = new THREE.Mesh(new THREE.SphereGeometry(0.18 + sm * 0.1, 6, 6), smokeMat);
...
function _buildFactoryDetails(scene) {
  ...
  for (const [bx, , bz] of barrelPositions) {
    const b = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 0.65, 1.5, 12), barrelMat);
    b.position.set(bx, 0.75, bz);
    scene.add(b);
    const band = new THREE.Mesh(new THREE.TorusGeometry(0.72, 0.05, 6, 16), bandMat);
  ...
  for (let ci = 0; ci < 3; ci++) {
    const can = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 0.9, 10), canMat);
...
function _buildOverheadPipes(scene) {
  ...
  const seg1 = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 22, 8), pipeMat);
  seg1.rotation.z = Math.PI / 2;
  seg1.position.set(-9, 11, 0);
  scene.add(seg1);

  // Destilator up
  const seg2 = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 5, 8), pipeMat);
  seg2.position.set(0, 8.5, -12);
  scene.add(seg2);

  // Destilator to Kompor Pemanas
  const seg3 = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 20, 8), pipeMat);
  ...
  for (const [px, pz] of [[-18, 6], [0, -12], [18, 6]]) {
    const drop = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 3, 8), pipeMat);
...
export function createFactoryQuestionObjects(scene) {
  ...
  for (const p of props) {
    ...
    const orb = new THREE.Mesh(new THREE.SphereGeometry(0.5, 12, 10), glowMat);
    orb.position.y = 4;
    grp.add(orb);

    // Halo ring
    const ringMat = new THREE.MeshStandardMaterial({ color: p.color, emissive: p.color, emissiveIntensity: 0.4 });
    const ring = new THREE.Mesh(new THREE.TorusGeometry(0.75, 0.06, 8, 28), ringMat);
